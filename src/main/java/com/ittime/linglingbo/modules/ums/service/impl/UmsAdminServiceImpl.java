...
import com.ittime.linglingbo.common.exception.Asserts;
import com.ittime.linglingbo.domain.AdminUserDetails;
...
import com.ittime.linglingbo.modules.ums.mapper.*;
import com.ittime.linglingbo.modules.ums.model.*;
import com.ittime.linglingbo.modules.ums.service.UmsAdminCacheService;
import com.ittime.linglingbo.modules.ums.service.UmsAdminRoleRelationService;
import com.ittime.linglingbo.modules.ums.service.UmsAdminService;
import com.ittime.linglingbo.security.util.JwtTokenUtil;
import com.ittime.linglingbo.security.util.SpringUtil;
...
public class UmsAdminServiceImpl extends ServiceImpl<UmsAdminMapper, UmsAdmin> implements UmsAdminService {
    ...
    @Override
    public boolean certificate(CertificateParam param) {
        int identity = param.getIdentity();
        // 品牌商
        if (identity == 1){
            QueryWrapper<UmsAdminOwner> wrapper = new QueryWrapper<>();
            wrapper.eq("real_name", param.getRealName())
                    .eq("id_card", param.getIdCard());
            UmsAdminOwner owner = ownerMapper.selectOne(wrapper);
            String certification = param.getStatus() == 1 ? "已认证" : "未认证";
            owner.setCertification(certification);
            ownerMapper.updateById(owner);
            return true;
        }
        // 主播
        if(identity == 2) {
            QueryWrapper<UmsAdminStreamer> wrapper = new QueryWrapper<>();
            wrapper.eq("real_Name", param.getRealName())
                    .eq("id_card", param.getIdCard());
            UmsAdminStreamer streamer = streamerMapper.selectOne(wrapper);
            String certification = param.getStatus() == 1 ? "已认证" : "未认证";
            streamer.setCertification(certification);
